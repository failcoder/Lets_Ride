...
_G.BINDING_NAME_LETS_RIDE = "Let's Ride";
--[[ 
########################################################## 
LOCAL VARIABLES
##########################################################
...
local function UpdateMountCheckboxes(button, index)
	...
	if(bar) then
		...
		if(DATA.Mounts.names["GROUND"] == creatureName) then
			if(DATA.Mounts.types["GROUND"] ~= index) then
				DATA.Mounts.types["GROUND"] = index
			end
		...
		if(DATA.Mounts.names["FLYING"] == creatureName) then
			if(DATA.Mounts.types["FLYING"] ~= index) then
				DATA.Mounts.types["FLYING"] = index
			end
		...
		if(DATA.Mounts.names["SWIMMING"] == creatureName) then
			if(DATA.Mounts.types["SWIMMING"] ~= index) then
				DATA.Mounts.types["SWIMMING"] = index
			end
		...
		if(DATA.Mounts.names["SPECIAL"] == creatureName) then
			if(DATA.Mounts.types["SPECIAL"] ~= index) then
				DATA.Mounts.types["SPECIAL"] = index
			end
...
local function UpdateMountsCache()
	...
	for index = 1, num, 1 do
		...
		if(DATA.Mounts.names["GROUND"] == info) then
			if(DATA.Mounts.types["GROUND"] ~= index) then
				DATA.Mounts.types["GROUND"] = index
			end
		end
		if(DATA.Mounts.names["FLYING"] == info) then
			if(DATA.Mounts.types["FLYING"] ~= index) then
				DATA.Mounts.types["FLYING"] = index
			end
		end
		if(DATA.Mounts.names["SWIMMING"] == info) then
			if(DATA.Mounts.types["SWIMMING"] ~= index) then
				DATA.Mounts.types["SWIMMING"] = index
			end
		end
		if(DATA.Mounts.names["SPECIAL"] == info) then
			if(DATA.Mounts.types["SPECIAL"] ~= index) then
				DATA.Mounts.types["SPECIAL"] = index
			end
		end
...
local function UpdateCurrentMountSelection()
	...
	if(DATA.Mounts.types["FLYING"]) then
		creatureName = DATA.Mounts.names["FLYING"]
		if(creatureName) then
			TOOLTIP_SUMMARY = TOOLTIP_SUMMARY .. "\nFlying: " .. creatureName
		end
	end

	if(DATA.Mounts.types["SWIMMING"]) then
		creatureName = DATA.Mounts.names["SWIMMING"]
		if(creatureName) then
			TOOLTIP_SUMMARY = TOOLTIP_SUMMARY .. "\nSwimming: " .. creatureName
		end
	end

	if(DATA.Mounts.types["GROUND"]) then
		creatureName = DATA.Mounts.names["GROUND"]
		if(creatureName) then
			TOOLTIP_SUMMARY = TOOLTIP_SUMMARY .. "\nGround: " .. creatureName
		end
	end

	if(DATA.Mounts.types["SPECIAL"]) then
		creatureName = DATA.Mounts.names["SPECIAL"]
		if(creatureName) then
...
local CheckButton_OnClick = function(self)
	...
	if(index) then
		if(self:GetChecked() == true) then
			DATA.Mounts.types[key] = index
			DATA.Mounts.names[key] = name
		else
			DATA.Mounts.types[key] = false
			DATA.Mounts.names[key] = ""
		end
...
local function CreateMountCheckBox(name, parent)
    ...
    if(frame.SetCheckedTexture) then
        ...
        frame.checked:SetTexture([[Interface\AddOns\Lets_Ride\assets\GLOSS]])
...
function CoreObject:Initialize()
	if(InterfaceVersion > 60000) then
		LoadAddOn("Blizzard_Collections")
	else
		LoadAddOn("Blizzard_PetJournal")
	end

	DATA = _G["LetsRide_Data"] or {};
	DATA.Mounts = DATA.Mounts or {};

	if not DATA.Mounts.types then 
		DATA.Mounts.types = {
			["GROUND"] = false, 
			["FLYING"] = false, 
			["SWIMMING"] = false, 
	...
	for i = 1, #buttons do
		...
		local height = 7
		local barName = ("LetsRideSelectionBar%d"):format(i)
...
local MountListener_OnEvent = function(self, event, arg, ...)
    ...
    elseif(event == "COMPANION_LEARNED" or event == "COMPANION_UNLEARNED") then
		UpdateMountsCache()
		Update_MountCheckButtons()
	elseif(event == "MOUNT_JOURNAL_USABILITY_CHANGED" or event == "COMPANION_UPDATE") then
		Update_MountCheckButtons()
    end
end

MountListener:RegisterEvent("PLAYER_LOGIN")
MountListener:SetScript("OnEvent", MountListener_OnEvent)
--[[ 
########################################################## 
SLASH FUNCTION
##########################################################
]]--
_G.PutMeOnMyDamnMount = function()
	...
	local checkList = DATA.Mounts.types
